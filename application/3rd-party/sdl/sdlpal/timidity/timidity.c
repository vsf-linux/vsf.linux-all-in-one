...
static int read_config_file(const char *name)
{
  ...
  while (RWgets(rw, tmp, sizeof(tmp)))
  {
	...
	goto fail;
      ...
      for (i=1; i<words; i++){
        if(UTIL_IsAbsolutePath(w[i]))
	        add_to_pathlist(w[i]);
        else
            add_to_pathlist(PAL_va(0, "%s%s%s", g_config_path, PAL_NATIVE_PATH_SEPARATOR, w[i]));
      }
    }
...
int Timidity_Init(char *config_path, char *config_file)
{
  ...
  if(UTIL_IsAbsolutePath(config_file)) {
    snprintf(g_config_path, 256, "%s", UTIL_basename(config_file));
    config_file=SDL_strrchr(config_file, PAL_NATIVE_PATH_SEPARATOR[0])+1;
  }else{
    memcpy(g_config_path, config_path, strlen(config_path));
    add_to_pathlist(g_config_path);
  }

  if (!env || read_config_file(env)<0) {
    if (read_config_file(UTIL_GetFullPathName(PAL_BUFFER_SIZE_ARGS(0), g_config_path, config_file))<0) {
    ...
    char rel_path[256] = "";
    SDL_snprintf(rel_path, 256, "%s%s%s", g_config_path, PAL_NATIVE_PATH_SEPARATOR, UTIL_basename(config_file));
    add_to_pathlist(rel_path);
  return 0;
}
