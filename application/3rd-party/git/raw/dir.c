...
{
	return __ignore_case ? strcasecmp(a, b) : strcmp(a, b);
}
...
{
	return __ignore_case ? strncasecmp(a, b, count) : strncmp(a, b, count);
}

unsigned int fspathhash(const char *str)
{
	return __ignore_case ? strihash(str) : strhash(str);
}
...
{
	if (index_file_exists(istate, pathname, len, __ignore_case))
		return NULL;
							 ...
							 const char *dirname, int len)
{
	...
	ce = index_file_exists(istate, dirname, len, __ignore_case);
